...
public class Barista implements BaristaInterface {
    ...
    private String name;
    private final Map<LocalDate, List<CoffeeOrder>> salesHistory = new HashMap<>();    // 판매 내역

    private Barista(String name) {
        this.name = name;
    }

    public static Barista createBarista(String name) {
        BaristaRepository.addBarista(new Barista(name));
        return BaristaRepository.getBarista(name);
    }
    ...
    public List<Coffee> makeCoffee(List<CoffeeOrder> coffeeOrders, LocalDate orderDate) {
        ...
        this.salesHistory.put(getLocalDate(orderDate), coffeeOrders); // 제조하면서 판매 내역에 담음.
        return coffees;
    }
